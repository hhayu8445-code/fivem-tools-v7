...
class FiveMDecryptor {
    ...
    async processLuaFile(decryptedBuffer, outputPath, resourceName, file) {
        ...
        const command = `java -jar "${decompilerPath}" "${tmpPath}"`;

        return new Promise((resolve) => {
            exec(command, { maxBuffer: 1024 * 1024 * 10 }, (error, stdout, stderr) => {
                if (!error && stdout) {
                    fs.writeFileSync(finalPath, stdout);
                } else {
                    const fileName = path.basename(finalPath, path.extname(finalPath));
                    const errorFilePath = path.join(path.dirname(finalPath), `${fileName}_bytecode.lua`);
                    fs.writeFileSync(errorFilePath, decryptedBuffer);
                }
                resolve();
            });
        });
